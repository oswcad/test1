...
class TruthTable:
    ...
    def kmap(self, idx, output):
        ...
        rtab = ttodraw(
            self.inames, self.get_minterms(output), self.get_dontcares(output)
        )
        lgroups = self.get_groups(output)
        with schemdraw.Drawing(canvas="matplotlib", show=False) as d:
            d.add(
                logic.Kmap(names="".join(self.inames), truthtable=rtab, groups=lgroups)
            )
    ...
    def circuit(self, idx, output):
        ...
        with schemdraw.Drawing(canvas="svg", show=False) as d:
            d = logic
...
help.write(
    """
Inputs: Write *up to 4 input* labels separated by commas. Each input label must be of one-character.
...
# inputs
input_labels = st.sidebar.text_input(
    "Input labels such as: A, B, C",
    key="input",
    placeholder="Separate multiple labels with commas",
)
...
if (n >= 1) and (n <= 4):
    # Outputs
    if (n > 0) and (inputs[0] != ""):
        output_labels = st.sidebar.text_input(
            "Outputs labels such as Co, S",
            key="output",
            placeholder="Separate multiple labels with commas",
        )
...
if (m >= 1) and (outputs[0] != ""):
    if dontcares:
        for ol in outputs:
            ...
            if len(common) > 0:
                st.sidebar.warning(
                    f"Row {common[0]} must be EITHER a minterm OR don'tcare"
                )
                st.stop()
...
if (m >= 1) and (outputs[0] != ""):
    with tab4:
        if st.session_state.msop_boolean:
            ...
            mycode = my_template.render(
                inputs=mytable.inames, outputs=mytable.onames, eqs=leqs
            )
